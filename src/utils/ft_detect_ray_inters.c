...
{
    t_cylinder *cyl = (t_cylinder *)(cy->obj_struct);
	double t;
	t_tuple point;
    double cap_y = cyl->height / 2.0;

    if ((fabs(ray.direction.y) < 1e-5))
        return ;
    t = (cap_y - ray.origin.y) / ray.direction.y;
    point = ft_rc_position(ray, t);
    if (sqrt((point.x * point.x) + (point.z * point.z)) <= cyl->diameter / 2.0)
...
void	ft_cylinder_inters(t_ray ray, t_oitem *cy, t_ray_inters **i_list)
{
	...
	if ((!isnan(tan[1]) && (!isnan(tan[0]) || tan[0] != tan[1])) && check_height(ray, tan[1], *cyl))

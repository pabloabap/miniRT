...
t_ray	ft_ray_for_pixel(t_camera camera, double px, double py)
{
	...
	pixel_p = ft_matrix_to_tuple(ft_matrix_mult(ft_inverse_matrix(\
		&(camera.transformations_matrix)), \
		ft_tuple_to_matrix(ft_build_tuple(world[X], world[Y], -1, POINT))));
	ray.origin = ft_matrix_to_tuple(ft_matrix_mult(ft_inverse_matrix(\
		&(camera.transformations_matrix)), \
